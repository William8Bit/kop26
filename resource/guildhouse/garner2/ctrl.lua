...
function init_entry(map)
    ...
    SetMapEntryTime(map, "2006/10/18/19/0", "0/3/0", "0/0/40", "0/0/45") --设置入口的时间，地图对象，首次开启时间（年/月/日/时/分），以后再次开启的间隔（日/时/分，全０表示只有首次开启），每次开启到入口消失的间隔（日/时/分，全０表示永不消失），每次开启到地图关闭的间隔（日/时/分，全０表示永不关闭）。。
...
function map_copy_run_special_garner2( map_copy ) 
	...
	Notice("<CA> There are current "..HowManyNoDead.." people in Chaos Argent")
...
--	SystemNotice(role,How_Many_Active)
	...
	if How_Many_Active <  6 and How_Many_Active > 1 then
		...
		if Item_CanGet <= 0 then
			...
			GiveItemX ( role , 0 , 10120  , 10 , 4 )
		else
			GiveItem ( role , 0 , 1032  , 1 , 4 )
			GiveItem ( role , 0 , 10120  , 10 , 4 )
		end
	...
	if How_Many_Active == 1 then
		local Item_CanGet = GetChaFreeBagGridNum ( role )
		if Item_CanGet <= 0 then
			GiveItemX ( role , 0 , 1032  , 3 , 4 )
			GiveItemX ( role , 0 , 10121  , 1 , 4 )
		else
			GiveItem ( role , 0 , 1032  , 3 , 4 )
			GiveItem ( role , 0 , 10121  , 1 , 4 )
		end
		AddMoney ( role , 0 , Money_Garner2 )
		local cha_name = GetChaDefaultName ( role )
		local message = "Congratulations "..cha_name.." you have earned ".. Money_Garner2 .." gold and 1 King of Ring Emblem"
	--	SystemNotice(role,How_Many_Active)
