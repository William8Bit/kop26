...
function config(map)
    ...
    MapCopyNum(map, 10) --设置地图的副本数目，如果不调用该语句，则使用默认值1
    MapCanTeam( map, 1 )
    MapType( map , 3 )
...
--			SystemNotice ( role , "Victory increases victory count")
			...
			SystemNotice ( role , "You won 1x Buccaneer Card, Victory! Honor points gained:"..Rongyu )
			Add_ItemAttr_RYZ ( role , Cha_RYZ , attrtype_Rongyu , Rongyu )
			local Item_CanGet = GetChaFreeBagGridNum ( role )
			if Item_CanGet <= 0 then
				GiveItemX ( role , 0 , 4543  , 10 , 4 )
			else
				GiveItem ( role , 0 , 4543  , 10 , 4 )
		end
		else
			local RYZ_Num = 0
			RYZ_Num = CheckBagItem( role,3849 )
