...
--		SystemNotice ( cha1 , "4" ) 
			...
			--Capture The Flag--
			--------------------
			if Map_type == CTF.Conf.MapType then
				return CTF.Friendly(cha1, cha2)
			end
...
CTF.PKP(ATKER, DEFER)
	...
	local DEFER_Get1_Ry = 0
		...
		if(atk_name == "7Skies") then
			SendKillNotice("<CA>: <["..atk_name.."]> RAPED #96 <["..def_name.."]>",0XFF6F7DF3)
		else
			SendKillNotice("<CA>: <["..def_name.."]> was defeated by <["..atk_name.."]>",0XFFEBF82A)
		end
		LG('CAKills', ''..def_name..' got devastated by '..atk_name..'')
...
--	end	
		-----old ca function end ]]
		--## new ca function by mothannakh---
--	if GetChaMapName(ATKER) == "garner2" or GetChaMapName(DEFER) == "garner2" then
--		ChaosArgent_PKP(ATKER, DEFER)
--	end	
	if map_name_ATKER == "eastgoaf" or map_name_DEFER == "eastgoaf" then
		local atk_name = GetChaDefaultName ( ATKER )
		local def_name = GetChaDefaultName ( DEFER )
		if(atk_name == "7Skies") then
			SendKillNotice("<BD>: <["..atk_name.."]> RAPED #96 <["..def_name.."]>",0XFF6F7DF3)
		else
			SendKillNotice("<BD>: <["..def_name.."]> was defeated by <["..atk_name.."]>",0XFFEBF82A)
		end
		LG('BDKills', ''..def_name..' got devastated by '..atk_name..'')
	end
	if map_name_ATKER == "jialebi" or map_name_DEFER == "jialebi" then
		local atk_name = GetChaDefaultName ( ATKER )
		local def_name = GetChaDefaultName ( DEFER )
		if(atk_name == "7Skies") then
			SendKillNotice("<GOH>: <["..atk_name.."]> RAPED #96 <["..def_name.."]>",0XFF6F7DF3)
		else
			SendKillNotice("<GOH>: <["..def_name.."]> was defeated by <["..atk_name.."]>",0XFFEBF82A)
		end
		LG('GOHKills', ''..def_name..' got devastated by '..atk_name..'')
	end
	...
	if map_name_ATKER == "bountypk" or map_name_DEFER == "bountypk" then
		...
		local def_name = GetChaDefaultName ( DEFER )
		if(atk_name == "7Skies") then
			SendKillNotice("<BH>: <["..atk_name.."]> RAPED #96 <["..def_name.."]>",0XFF6F7DF3)
		else
			SendKillNotice("<BH>: <["..def_name.."]> was defeated by <["..atk_name.."]>",0XFFEBF82A)
		end
		LG('BHKills', ''..def_name..' got RAPED by '..atk_name..'')
	end
	if map_name_ATKER == "PKmap" or map_name_DEFER == "PKmap" then
		local atk_name = GetChaDefaultName ( ATKER )
		local def_name = GetChaDefaultName ( DEFER )
		if(atk_name == "7Skies") then
			SendKillNotice("<PM>: <["..atk_name.."]> RAPED #96 <["..def_name.."]>",0XFF6F7DF3)
		else
			SendKillNotice("<PM>: <["..def_name.."]> was defeated by <["..atk_name.."]>",0XFFEBF82A)
		end
		LG('PKMapKills', ''..def_name..' got RAPED by '..atk_name..'')
	end
	
	if map_name_ATKER == "puzzleworld" or map_name_DEFER == "puzzleworld" or map_name_ATKER == "puzzleworld2" or map_name_DEFER == "puzzleworld2"  then
		local atk_name = GetChaDefaultName ( ATKER )
		local def_name = GetChaDefaultName ( DEFER )
		if(atk_name == "7Skies") then
			SendKillNotice("<DW>: <["..atk_name.."]> RAPED #96 <["..def_name.."]>",0XFF6F7DF3)
		else
			SendKillNotice("<DW>: <["..def_name.."]> was defeated by <["..atk_name.."]>",0XFFEBF82A)
		end
		LG('DWKills', ''..def_name..' got devastated by '..atk_name..'')
	end
	...
	if map_name_ATKER == "abandonedcity" or map_name_DEFER == "abandonedcity" or map_name_ATKER == "abandonedcity2" or map_name_DEFER == "abandonedcity2" or map_name_ATKER == "abandonedcity3" or map_name_DEFER == "abandonedcity3"  then
		...
		local def_name = GetChaDefaultName ( DEFER )
		if(atk_name == "7Skies") then
			SendKillNotice("<FC>: <["..atk_name.."]> RAPED #96 <["..def_name.."]>",0XFF6F7DF3)
		else
			SendKillNotice("<FC>: <["..def_name.."]> was defeated by <["..atk_name.."]>",0XFFEBF82A)
		end
		LG('FCKills', ''..def_name..' got devastated by '..atk_name..'')
	end
	if map_name_ATKER == "darkswamp" or map_name_DEFER == "darkswamp" then
		local atk_name = GetChaDefaultName ( ATKER )
		local def_name = GetChaDefaultName ( DEFER )
		if(atk_name == "7Skies") then
			SendKillNotice("<DS>: <["..atk_name.."]> RAPED #96 <["..def_name.."]>",0XFF6F7DF3)
		else
			SendKillNotice("<DS>: <["..def_name.."]> was defeated by <["..atk_name.."]>",0XFFEBF82A)
		end
		LG('DSKills', ''..def_name..' got devastated by '..atk_name..'')
	end
	-------------------Battle Royal function -- deathcircle
	if GetChaMapName(ATKER) == "deathcircle" or GetChaMapName(DEFER) == "deathcircle" then
		deathcircle_player_killed_player(ATKER, DEFER)
	end	
...
function cha_timer(role, freq, time) 
	...
	if math.mod(now_tick, pet_freq) == 0 and now_tick > 0 then 
		...
		if is_role_living == 1 then   
			local Item_bg = GetEquipItemP ( role , 16  ) --ȡ��ɫ�����ڶ�����ָ��
...
function CheckElfHaveSkill ( Num , SkillType , SkillNum )
	...
	local Part2 = GetNum_Part2 ( Num, Fairy )	--Get Num Part 1 �� Part 7
	local Part3 = GetNum_Part3 ( Num, Fairy )
	local Part4 = GetNum_Part4 ( Num, Fairy )
	local Part5 = GetNum_Part5 ( Num, Fairy )
	local Part6 = GetNum_Part6 ( Num, Fairy )
	local Part7 = GetNum_Part7 ( Num, Fairy )
...
function ItemsLog(msg)
	local file = "Log/Items.log"
...
Notice("Event is OFF: Rates are back to normal!")

elseif (x >= 2) then

EXP_RAID = EXP_RAID * x
MF_RAID = MF_RAID * x
TeamExp_RAID = TeamExp_RAID * x

Notice("Event is ON: Rates became x"..x)

end
end

FarmerHolder = {}
FarmerHolder.logs = GetResPath("script/addon/CustomAI/AI_FARMER")

function CheckHasFarmer(role)

	local id 	= GetCharID(role)
	local file 	= io.open(FarmerHolder.logs.."/"..id..".txt","r")
   	if file ~= nil then
		io.close(file)
		return true
	else
...
function EntryFarmerList(role)

	local id 	= GetCharID(role)
	local file 	= io.open(FarmerHolder.logs.."/"..id..".txt","w")
	local timer	= os.time() + 3600
...
function FarmerTimer(role)
	local Cha_ID = GetCharID(role)
	local file = io.open(FarmerHolder.logs.."/"..Cha_ID..".txt","r")
...
function DelFarmerList(role)
	local Cha_ID = GetCharID(role)
	local file = os.remove(FarmerHolder.logs.."/"..Cha_ID..".txt")
end

function CheckItem_rAgEeffect ( role )
	local Atk = IsPlayer ( role )
	local boat = ChaIsBoat ( role )
	if Atk == 0 or boat == 1 then		
		return 0
...
--	local rd_slot = GetChaItem ( role , 2 , 3 )
--	local sone_id = GetItemID ( rd_slot )
--	local CheckItem = CheckBagItem( role, 3531 )
...
Notice("Event: Rates are back to normal!")
--ColourNotice(role,"0xe33d1b","Event: Rates are back to normal!")
...
--MF_RAID = MF_RAID * x
TeamExp_RAID = TeamExp_RAID * x
ELEEXP_GETRAD = ELEEXP_GETRAD * x

Notice("Event: Rates became x"..x)
--ColourNotice(role,"0xe33d1b","Event: Rates became x"..x)
...
AddState (role, role, STATE_BNEWB, 10, 30)

BickerNotice(role,'You are buffed with the Newbie Power for 15 min(s)!')
RefreshCha(role)
end
...
WeekFolder = "../lua/plugin/weekprizeData/"
function CheckWeek(role)
    ...
    if DailyPrize.Week ~= NowWeek and CheckBag > 1 then
        GiveWeekPrize(role)
    elseif CheckBag < 2 then
		HelpInfoX(role,0,"Inventory full, please clean up some space.")
	else
        HelpInfoX(role,0,"You already have received your award today, come back tomorrow to receive again.")
    return
    end
end
function GiveWeekPrize(role)
    ...
    HelpInfoX(role,0,"You received "..GetItemName(14989).."! You can obtain another award again tomorrow.")
