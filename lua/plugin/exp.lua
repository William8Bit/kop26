...
--Use this as reference for New Sight Item that shows you Nearby Chest area of 5000 distance.
...
function BsAttrUpgrade(role)
	...
	--ap_extre = 3
	-- Each lv(from lv 60) player will get 1 stats points
	 if lv >= 1 then
		 ap_extre = ap_extre + 1
	 end
...
--	 Each 5 lvs(from lv 65) player will get 3 skill points, otherwise, only 2
	 if lv >= 65 then
...
function ShareTeamExp ( dead , team_atker , dead_exp , The_Killer) 
	...
	for i = 0 , 4 , 1 do
		if ValidCha(t[i])==1 then
			...
			---ocal playerLv = Lv(TurnToCha(t[i]))
			---f(Lv(dead) >(playerLv + 11))then
			---	if exp_up ~= 0 then
			---		exp_up = 0
			...
			if Lv ( TurnToCha(t[i]) ) > 60 and  Lv ( TurnToCha(t[i]) ) < 66 then 
				exp_up = exp_up/2
			end
			if Lv ( TurnToCha(t[i]) ) > 65 and Lv ( TurnToCha(t[i]) ) < 71 then 
				exp_up = exp_up/3
			end
...
function expbyunity(cha, exp_up)
	...
	if IdItem == 1034 and lvPerson < 51 then
