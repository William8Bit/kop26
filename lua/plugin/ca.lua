...
_G['map_copy_run_'..garner2.conf['mapName']] = function(map_copy)
  DealAllPlayerInMap(map_copy,"Garner2SearchPartyCheater")
...
_G['can_open_entry_'..garner2.conf['mapName']] = function(map) 
  local hour = tonumber(os.date("%H"))
  if hour == 6 or hour == 12 or hour == 18 or hour == 00 then
    return 1
  else
    return 0
  end
...
_G['check_can_enter_'..garner2.conf['mapName']] = function(role, copy_mgr)
	...
	local MacCheck = AltCheck.check(role,'garner2')
	if MacCheck ~= 0 then
		SystemNotice(role,"You already have a character inside Chaos Argent. Entry denied")
		return 0
	end
...
function GiveSpecItem6(role)
  if How_Many_Active == 0 then
    GMNotice('There were no winners this Chaos Argent!')
    return
  end
  ...
  if How_Many_Active == 1 then
    ...
    AddMoney(role, 0, Money_garner2)
    GMNotice(garner2.entry['mapName']..' <Only Survivor> '..GetChaDefaultName(role)..'!')
    return
  end
end 
...
garner2.spawn = function(map_copy, wave)
  ...
  local h = GetMapActivePlayer(map_copy)
  GMNotice('[CA] wave ['..Every_5_minute..'] spawned. There are '..h..' players in Chaos Argent')
